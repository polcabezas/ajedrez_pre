...
def test_solicitar_movimiento_cpu_sin_movimientos_log():
    """Verifica que se registra un warning si no hay movimientos (usando context manager)."""
    jugador = JugadorCPU("Test CPU", "negro")
    mock_juego = MockJuego()
    mock_juego.tablero.obtener_todos_movimientos_legales = MagicMock(return_value=[])

    # Usar patch como context manager
    with patch('model.jugadores.jugador_cpu.logger') as mock_logger:
